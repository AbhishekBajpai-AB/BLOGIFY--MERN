...
function Post({ post }) {
  ...
  const handleOnLike = () => {
    ...
    let updatedPost = {}
    const existingLike = safeLikes.filter((like) => like === user.result._id)
    if (existingLike.length === 0) {
      updatedPost = { ...post, likes: [...safeLikes, user.result._id] }
    } else {
      updatedPost = {
        ...post,
        likes: safeLikes.filter((like) => like !== user.result._id),
      }
    }
